...
If the ConfigMap with name `name` does not exist on VarnishService creation, the operator will create one and populate it with a default `<backendsFile>.tmpl` and `<defaultFile>`. Their behavior are as follows:
