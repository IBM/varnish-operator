...
func (q *ReconcileTriggerer) TriggerAfter(key string, triggerAfter time.Duration, instance *icmapiv1alpha1.VarnishService) {
...
func (q *ReconcileTriggerer) Stop(key string, instance *icmapiv1alpha1.VarnishService) {
	namespacedName := instance.Namespace + "/" + instance.Name
	if timers, exists := q.timetable[namespacedName]; exists {
		if timer, exists := timers[key]; exists {
			timer.Stop()
...
func (q *ReconcileTriggerer) TimerExists(key string, service *icmapiv1alpha1.VarnishService) bool {
